...
// Если во время ожидания сегмента, ожидание завершается абортом или ошибкой,
// этот сегмент не перезапускается. Но плеер получает соответствующее уведомление,
// на что может соотвутствующе реагировать, а может и не реагировать,
// допустим если закончались повторные попытки или реализация работает не так как ожидалось.

// Обработчик ошибок для промиса сегмента устанавливается в момент вызова функции "segment.abort()",
// если ошибка произойдёт до вызова функции "segment.abort()", обработчик ошибок "Segment.on_error()" не будет вызван,
// хоть это и может повлиять только на отсутствие логов.

// Ксли ОЖИДАЕМЫЙ сегмент завершится ошибкой - плеер получит соответствующие уведомление,
// и (возможно) перезапустит сегмент. В то время как завершение ошибкой НЕ ОЖИДАЕМЫХ сегментов их не перезапускает,
// и даже не удаляет из очереди.
// В теории плеер рано или поздно сам запросит этот сегмент, но так как промис уже был rejected,
// то это ожидания может стать потенциально бесконечным.

// Разрыв сети как триггер.
